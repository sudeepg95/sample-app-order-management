...
export const ExceptionReportingModal: React.FC<ExceptionReportingModalProps> = ({ 
  item, 
  isOpen, 
  onClose, 
  onSubmit 
          ...
          >
            ...
            <div className="bg-alert-red border-b-4 border-pitch-black px-8 py-6 flex justify-between items-center text-white">
              <div className="flex items-center gap-5">
                <AlertTriangle size={48} fill="white" className="text-alert-red" />
                <h2 className="font-headline text-4xl font-black uppercase tracking-tight">REPORT EXCEPTION</h2>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="p-10 flex flex-col lg:flex-row gap-12 overflow-y-auto">
              ...
              <div className="lg:w-1/3 flex flex-col items-center">
                <div className="relative w-full bg-white border-4 border-pitch-black pt-12 px-6 pb-6 shadow-hard">
                  ...
                  <div className="flex flex-col gap-2 mb-6">
                    <span className="font-mono text-[10px] font-black uppercase opacity-60 tracking-widest leading-none">SKU CODE</span>
                    <span className="font-mono text-xl font-black tracking-tighter leading-none">{item.product.sku}</span>
                  </div>
                  ...
                  <div className="pt-6 border-t-2 border-pitch-black/20">
                    ...
                    <div className="bg-primary-yellow border-4 border-pitch-black p-4 flex flex-col gap-1 shadow-hard-sm">
                      <span className="font-mono text-[10px] font-black uppercase opacity-60 leading-none">BIN IDENTITY</span>
                      <span className="font-headline text-3xl font-black tracking-widest text-pitch-black leading-none">{binLocation}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:w-2/3 flex flex-col">
                <p className="font-body text-lg mb-10 font-bold opacity-80 leading-relaxed max-w-xl">
                  Please classify the anomaly with the highlighted inventory item. This will alert shift command and suspend batch processing for STATION-04.
                </p>

                <div className="flex flex-col gap-10">
                  {/* Issue Classification */}
                  <div className="space-y-4">
                    <label className="block font-mono text-sm font-black uppercase tracking-widest opacity-60">CLASSIFY ISSUE (INPUT REQUIRED)</label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {issueTypes.map((type) => (
                        <button
                          ...
                          `}
                        >
                          <span className="font-mono text-sm font-black uppercase tracking-wider">{type}</span>
                          {selectedType === type && <Check size={20} className="text-pitch-black" />}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Operational Notes */}
                  <div className="space-y-4">
                    <label className="block font-mono text-sm font-black uppercase tracking-widest opacity-60">OPERATOR NOTES (OPTIONAL LOG)</label>
                    <textarea 
                      value={notes}
